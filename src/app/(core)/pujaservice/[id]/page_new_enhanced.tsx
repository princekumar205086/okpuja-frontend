...
import { 
  ...
  Sparkles, Timer, Gift, Info, ChevronLeft, ChevronRight
} from 'lucide-react';
...
export default function ServiceDetailPage() {
  ...
  const [relatedServices, setRelatedServices] = useState<PujaService[]>([]);
  ...
  useEffect(() => {
    const loadService = async () => {
      ...
      try {
        ...
        if (foundService) {
          const related = mockPujaServices.filter(s => 
            s.category.id === foundService.category.id && s.id !== foundService.id
          ).slice(0, 8);
  ...
  // Carousel controls
  const nextSlide = () => {
    setCurrentSlide((prev) => (prev + 1) % Math.ceil(relatedServices.length / 4));
  };

  const prevSlide = () => {
    setCurrentSlide((prev) => (prev - 1 + Math.ceil(relatedServices.length / 4)) % Math.ceil(relatedServices.length / 4));
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-orange-50 via-white to-orange-50">
      ...
      >
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1">
            <motion.div 
              ...
              className="bg-white rounded-3xl shadow-xl p-8 sticky top-24 mb-8"
            >
                      ...
                      >
                        ...
                        <div className="flex justify-between items-start mb-3">
                          <div className="flex items-center">
                            ...
                            <div>
                              <h4 className="font-bold text-gray-900 text-lg">{packageTypeDisplayNames[pkg.package_type]}</h4>
                        ...
                        <p className="text-sm text-gray-600 leading-relaxed" dangerouslySetInnerHTML={{ __html: pkg.description }} />
        ...
        {relatedServices.length > 0 && (
          <motion.div 
            variants={itemVariants}
            className="mt-16 bg-white rounded-3xl shadow-xl p-8 sm:p-10"
          >
            <div className="flex items-center justify-between mb-8">
              <h2 className="text-2xl sm:text-3xl font-bold text-gray-900">Related Services</h2>
            ...
            <div className="overflow-hidden">
              ...
              >
                {Array.from({ length: Math.ceil(relatedServices.length / 4) }).map((_, slideIndex) => (
                  <div key={slideIndex} className="w-full flex-shrink-0">
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
                      {relatedServices.slice(slideIndex * 4, (slideIndex + 1) * 4).map((relatedService, index) => (
                        <ServiceCard 
