...
const BookingsPage: React.FC = () => {
  ...
  const handleCancelBooking = async (booking: Booking) => {
    try {
      // Add API call to cancel booking here
      toast.success('Booking cancellation request sent!');
      handleRefresh();
    } catch (error) {
  ...
  // Computed values
  const pujaBookingsMapped: Booking[] = pujaBookings.map((b: any) => ({
    ...b,
    id: b.id?.toString?.() ?? b.book_id?.toString?.() ?? '',
  }));
  const astrologyBookingsMapped: Booking[] = astrologyBookings.map((b: any) => ({
    ...b,
    id: b.id?.toString?.() ?? b.astro_book_id?.toString?.() ?? '',
  }));
